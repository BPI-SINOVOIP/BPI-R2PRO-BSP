...
public:
    ...
    {		
        ...
        ExtObject(int _id, Rect_T _location, Rect_T _predict_loc_when_miss, Rect_T _smooth_rect, ObjectStatus _status, 
						int _detectedNum, int _noDetectedNum, int _obj_class, int _miss)
            :id(_id), location(_location), predict_loc_when_miss(_predict_loc_when_miss), smooth_rect(_smooth_rect), status(_status),
            		detectedNum(_detectedNum), noDetectedNum(_noDetectedNum), obj_class(_obj_class), miss(_miss)
        {
  ...
  /*  bool setParameters(const Parameters& params);
    ...
    void updateTrackedObjects(int maxTrackLifetime, const std::vector<Rect_T>& detectedObjects, const std::vector<int> objects_class, int image_width,int image_height);
    ...
    struct TrackedObject
    {
        ...
        TrackedObject(const Rect_T& Rect_T):vx(0), vy(0), numDetectedFrames(1), numFramesDetected(1), numFramesNotDetected(0)
        							, preNumFramesNotDetected(0), miss(0)
