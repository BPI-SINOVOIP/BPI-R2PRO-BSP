...
void ObjectsTracker::predict_loctation(TrackedObject& curObject, int image_width,int image_height, float& pre_x, float& pre_y, float& v_x, float& v_y){
	...
	float vx_ = (prevRect.x +prevRect.width*0.5f -tmpRect.x -tmpRect.width*0.5f)/(1.0f +curObject.preNumFramesNotDetected);
	float vy_ = (prevRect.y +prevRect.height*0.5f -tmpRect.y -tmpRect.height*0.5f)/(1.0f +curObject.preNumFramesNotDetected);
	v_x = 0.5f*vx_ + 0.5f*curObject.vx;
	v_y = 0.5f*vy_ + 0.5f*curObject.vy;

	int x = prevRect.x  + v_x*(1 +curObject.numFramesNotDetected) *0.7f +0.5f;
	int y = prevRect.y  + v_y*(1 +curObject.numFramesNotDetected) *0.7f +0.5f;
	x = x >= 0 ? x : 0;
	y = y >= 0 ? y : 0;
...
void ObjectsTracker::getObjects(std::vector<ExtObject>& result)
{
    ...
    for(size_t i=0; i < trackedObjects.size(); i++) {
        ...
        result.push_back(ExtObject(trackedObjects[i].id, r, trackedObjects[i].predict_loc_when_miss, trackedObjects[i].smooth_Positionn, status, 
			trackedObjects[i].numFramesDetected, trackedObjects[i].numFramesNotDetected, trackedObjects[i].obj_class, trackedObjects[i].miss));

	}
}

void ObjectsTracker::updateTrackedObjects(int maxTrackLifetime, const std::vector<Rect_T>& detectedObjects, const std::vector<int> objects_class
	, int image_width,int image_height)
{
    ...
    for(int i=0; i < N1; i++) {
        ...
        for(int j=0; j < N2; j++) {
			...
			//printf("search percentage_IOU%f detect_cla%d track_cla%d\n", percentage_IOU, objects_class[j], curObject.obj_class);
    ...
    for(int j=0; j < N2; j++) {
        ...
        if (i >= 0) {//add position
			...
			int _N1 = trackedObjects.size();
			trackedObjects[_N1-1].obj_class = objects_class[j];
			correspondence[j] = _N1 -1;
			
			trackedObjects[_N1-1].smooth_Positionn.width = detectedObjects[j].width;
            trackedObjects[_N1-1].smooth_Positionn.height = detectedObjects[j].height;
            trackedObjects[_N1-1].smooth_Positionn.x = (detectedObjects[j].x);
            trackedObjects[_N1-1].smooth_Positionn.y = (detectedObjects[j].y);
        }
    }
