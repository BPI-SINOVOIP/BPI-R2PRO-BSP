...
void adec_dump_stream_info(AVCodecParameters *cpar) {
...
void adec_ffmpeg_format_close(AVFormatContext *avformat) {
    if (avformat) {
        avformat_close_input(&(avformat));
    }
}

AVFormatContext *adec_ffmpeg_format_open(TEST_ADEC_CTX_S *params) {
    RK_S32 ret = 0;
    RK_S32 err = 0;
    RK_S32 retryNum = 5;
...
__RETRY:
    ...
    return avformat;
__FAILED:
    adec_ffmpeg_format_close(avformat);
    return RK_NULL;
}

static RK_S32 adec_ffmpeg_free(void *opaque) {
...
static RK_U32 test_find_audio_codec_id(TEST_ADEC_CTX_S *params) {
    ...
    if (params->s32DecMode == ADEC_MODE_STREAM) {
        ...
        // from ffmpeg, see adec_ffmpeg_format_open
...
RK_S32 test_init_mpi_adec(TEST_ADEC_CTX_S *params) {
    ...
    avfmt = adec_ffmpeg_format_open(params);
    if (!avfmt) {
        RK_LOGE("failed to ffmpeg format open file: %s", params->srcFilePath);
        return RK_FAILURE;
    }
    params->avformat = avfmt;
...
RK_S32 test_deinit_mpi_adec(TEST_ADEC_CTX_S *ctx) {
    ...
    adec_ffmpeg_format_close(ctx->avformat);
    return RK_SUCCESS;
}
...
static void *send_stream_thread(void *arg) {
    ...
    RK_S32 count = 0;
    RK_S64 dataOffset = 0;
    AVFormatContext *avformat = params->avformat;
    ...
    if (avformat->pb) {
        dataOffset = avio_tell(avformat->pb);
        if (dataOffset > 0) {
...
__RETRY:
        ...
        err = av_read_frame(avformat, avPacket);
        if (err == AVERROR_EOF || err == AVERROR_EXIT) {
            srcData = RK_NULL;
            srcSize = 0;
        } else {
            stream  = avformat->streams[avPacket->stream_index];
            if (stream->codecpar->codec_type == AVMEDIA_TYPE_AUDIO) {
                ...
                srcData = RK_NULL;
                srcSize = 0;
                adec_ffmpeg_free(avPacket);
                goto __RETRY;
            }
        }

        RK_LOGV("srcData = %p, srcSize = %d", srcData, srcSize);
        if (srcSize == 0 || srcData == RK_NULL) {
            RK_LOGI("read eos packet, now send eos packet!");
            pktEos = 1;
        }
        ...
        if (pktEos) {
            ...
            stAudioStream.bBypassMbBlk = RK_TRUE;
            MB_EXT_CONFIG_S extConfig = {0};
            extConfig.pFreeCB = adec_ffmpeg_free;
...
RK_S32 unit_test_mpi_adec(TEST_ADEC_CTX_S *params) {
    ...
    for (i = 0; i < params->s32ChnNum; i++) {
        ...
        if (params->s32DecMode == ADEC_MODE_STREAM) {
            ...
            pthread_create(&tidSend[i], RK_NULL, send_pack_thread, reinterpret_cast<void *>(&adecCtx[i]));
        }
