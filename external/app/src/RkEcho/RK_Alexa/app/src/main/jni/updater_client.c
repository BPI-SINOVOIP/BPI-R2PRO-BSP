...
#define MSG_VERSION_GET		0x0010
#define MSG_VERSION_OFFER	0x0020
#define MSG_VERSION_SET		0x0040
...
static char* device_ip_address = "192.168.100.1";
...
int updater_setmode(int sockfd, char* info_buf, unsigned short new_mode, unsigned short new_id)
{
    ...
    /* 模式合法性检查. bit15-bt14, bit4-bit0为有效位,其他强制为0. */
    if(new_mode & 0x3FE0)
    {
...
int sending_fw_datas(int sockfd, MsgFwRequest* param)
{
...
int updater_start(int sockfd, char* info_buf,JNIEnv *env)
{
    ...
    while(1)
    {
        ...
        if(hdr->type == htons(MSG_FW_REQUST))
        {
            ...
            ret = sending_fw_datas(sockfd, fwRequset);
            if(ret < 0)
                return ret;
        }
