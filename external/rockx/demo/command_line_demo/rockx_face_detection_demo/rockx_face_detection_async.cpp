...
void face_det_callback_func(void *result, size_t result_size, void *extra_data) {
...
int main(int argc, char** argv) {
    ...
    callback.callback_func = face_det_callback_func;
    callback.extra_data = nullptr;

    gettimeofday(&tv1, NULL);
    printf("%ld before rockx_face_detect\n", (tv1.tv_sec * 1000000 + tv1.tv_usec));
    ret = rockx_face_detect(face_det_handle, &input_image, &face_array, &callback);
    gettimeofday(&tv2, NULL);
    printf("%ld after rockx_face_detect\n", (tv2.tv_sec * 1000000 + tv2.tv_usec));
