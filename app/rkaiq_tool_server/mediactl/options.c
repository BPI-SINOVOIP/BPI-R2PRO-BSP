...
static void usage(const char* argv0) {
  ...
  printf(
      "-e, --entity name	Print the device name associated with the "
      "given "
      "entity\n");
  printf("-V, --set-v4l2 v4l2	Comma-separated list of formats to setup\n");
  printf("    --get-v4l2 pad	Print the active format on a given pad\n");
  printf(
      "    --get-dv pad        Print detected and current DV timings on a "
      "given pad\n");
  printf("    --set-dv pad	Configure DV timings on a given pad\n");
  printf("-h, --help		Show verbose help and exit\n");
  printf("-i, --interactive	Modify links interactively\n");
  printf(
      "-l, --links links	Comma-separated list of link descriptors to "
      "setup\n");
  printf(
      "    --known-mbus-fmts	List known media bus formats and their "
      "numeric values\n");
  ...
  printf(
      "    --print-dot		Print the device topology as a dot "
      "graph\n");
...
int parse_cmdline(int argc, char** argv) {
  ...
  while ((opt = getopt_long(argc, argv, "d:e:f:hil:prvV:", opts, NULL)) != -1) {
    switch (opt) {
      ...
      case 'f':
        fprintf(stderr,
                "Warning: the -f option is deprecated "
