...
static int udev_fill_info(struct device_info *info, struct stat *stat)
{
	...
	LOG_INFO("udev_fill_info()\n");

    ctx = udev_new();
    if (!ctx) {
	if (device_info_verbose)
	    LOG_INFO("no udev library context\n");
	return -1;
    }
     ...
     */
    if (device_info_verbose >= 3)
	LOG_INFO("looking for dependent devices\n");
    ...
    if (!parent) {
	if (device_info_verbose >= 3)
	    LOG_INFO("no parent found, therefore virtual device\n");
    ...
    if (device_info_verbose >= 3) {
	if (attr)
	    LOG_INFO("attribute \"removable\" is \"%s\"\n", attr);
	else
	    LOG_INFO("attribute \"removable\" not found\n");
    }
