...
#ifdef WAKEUP_FESPA
#include "wakeup_fespa_aircondition.c"
#elif defined(WAKEUP_FESPL)
#include "wakeup_fespl_aircondition.c"
#endif
...
static void speech_to_wait_for_wakeup(void *args) {
    ...
    m.type = PLAYER_CMD_PLAY;
    m.player.mode = PLAYER_MODE_PROMPT;
    m.player.target = "./audio/network_error.wav";
    m.player.native = true;
    m.player.need_free = false;
    m.player.end_session = true;
    os_queue_send(player_queue, &m); 
...
static int dui_parse_cfg(const char *cfg) {
    ...
    cJSON *wakeup_param_js = cJSON_GetObjectItem(wakeup_js, "param");
    #ifdef WAKEUP_AEC
    cJSON *aec_cfg_js = cJSON_GetObjectItem(wakeup_js, "aecCfg");
    cJSON *aec_param_js = cJSON_GetObjectItem(wakeup_js, "aecParam");
    g_cfg.wakeup.aec_cfg = cJSON_Print(aec_cfg_js);
    g_cfg.wakeup.aec_param = cJSON_Print(aec_param_js);
    #endif
    ...
    g_cfg.wakeup.param = cJSON_Print(wakeup_param_js);
...
void dui_library_cleanup(void) {
    ...
    #ifdef WAKEUP_AEC
    dui_free(g_cfg.wakeup.aec_cfg);
    dui_free(g_cfg.wakeup.aec_param);
    #endif
    int i;
    for (i = 0; i < g_cfg.wakeup.word_count; i++) {
