...
unsigned int mii_mgr_read(unsigned int phy_addr,unsigned int phy_register,unsigned int *read_data)
{
	...
	*read_data = bus->read(bus, phy_addr, phy_register);
	/* printk("%s, %d, PHY ADDR: 0x%08x, REG ADDR: 0x%08x, VALUE: 0x%08x \n", __FUNCTION__, __LINE__, phy_addr, phy_register, *read_data); */
...
unsigned int mii_mgr_write(unsigned int phy_addr,unsigned int phy_register,unsigned int write_data)
{
	...
	/* printk("%s, %d, PHY ADDR: 0x%08x, REG ADDR: 0x%08x, VALUE: 0x%08x \n", __FUNCTION__, __LINE__, phy_addr, phy_register, write_data); */
...
static int rtl8367s_hw_reset(void)
{
	...
	ret = devm_gpio_request(gsw->dev, gsw->reset_pin, "rockchip,reset-pin");
...
static void set_rtl8367s_rgmii_ep0(void)
{
	rtk_port_mac_ability_t mac_cfg;
	rtk_mode_ext_t mode;
...
static void set_rtl8367s_rgmii_ep1(void)
{
	rtk_port_mac_ability_t mac_cfg;
	rtk_mode_ext_t mode;
...
{
	if (rtl8367s_hw_init())
		printk("BPI-R2PRO RK3568's RTL8367RB Switch Init Failure !!!\n");
	else
		printk("BPI-R2PRO RK3568's RTL8367RB Switch Init Successfully !!!\n");

	/* rtl8367s_hw_init(); */
	/* set_rtl8367s_sgmii(); */
	set_rtl8367s_rgmii_ep0();
	set_rtl8367s_rgmii_ep1(); 
}
...
MODULE_LICENSE("GPL");
MODULE_AUTHOR("zuowei <zuowei8@tom.com>");
MODULE_DESCRIPTION("rtl8367c switch driver for RK3568");
